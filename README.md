...
## Uso (UX do MVP)
- Colaboradoras usam apenas **botões** via `/start`.
- O bot **não pergunta papel**: por padrão registra como `corretora`.
- Gerência entra no módulo administrativo com **`/admin`** (ID precisa estar em `ADMIN_TELEGRAM_IDS`).

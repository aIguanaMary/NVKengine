...
def _is_manager(telegram_id: int, role: str, settings: Settings) -> bool:
    return telegram_id in settings.admin_telegram_ids or role in {"gerente", "admin", "gestor"}


async def register_user_access(update: Update, context: ContextTypes.DEFAULT_TYPE, role_override: str | None = None) -> Dict[str, str]:
    ...
    return await asyncio.to_thread(repo.upsert_user, telegram_id=int(info["telegram_id"]), username=info["username"], name=info["name"], role=role_override, now_str=now)
...
async def show_state(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    role = (user_row or {}).get("PAPEL", "").strip().lower()

    if state == "SET_ROLE":
        rows = [[InlineKeyboardButton(lbl, callback_data=f"role:{key}")] for key, lbl in ROLE_OPTIONS]
        rows.append([InlineKeyboardButton("❌ Cancelar", callback_data="nav:cancel")])
        await render_menu(update=update, context=context, session=session, text="Bem-vindo(a)! Para registrar suas ações corretamente, selecione seu papel:", keyboard=rows)
        return
    ...
    if state == "MAIN_MENU":
        ...
        if _is_manager(update.effective_user.id, role, settings):
            keyboard.append([InlineKeyboardButton("🛠️ Gerência", callback_data="m:manager")])
        keyboard.append([InlineKeyboardButton("👤 Meu papel", callback_data="m:role")])
        text = f"🏠 Bot Interno da Imobiliária\n\nUse os botões abaixo.\nSeu papel atual: {(role or 'não definido').upper()}"
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await register_user_access(update, context)
    session = get_session(update.effective_user.id)
    user = await asyncio.to_thread(context.application.bot_data["repo"].get_user, update.effective_user.id)
    reset_flow(session)
    session["state"] = "SET_ROLE" if (not user or not (user.get("PAPEL") or "").strip()) else "MAIN_MENU"
    await show_state(update, context)
...
async def on_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if state == "VISIT_NEW_OBS":
        ...
        user = await asyncio.to_thread(repo.get_user, user_id) or {}
        payload = {
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data.startswith("role:"):
        await register_user_access(update, context, role_override=data.split(":",1)[1])
        reset_flow(session); await show_state(update, context); return
    ...
    if data == "m:manager":
        user = await asyncio.to_thread(repo.get_user, update.effective_user.id) or {}
        if not _is_manager(update.effective_user.id, (user.get("PAPEL","") or "").lower(), settings):
            await q.answer("Acesso restrito à gerência.", show_alert=True); return
        push_history(session); session["state"] = "MANAGER_MENU"; await show_state(update, context); return
    if data == "m:role":
        push_history(session); session["state"] = "SET_ROLE"; await show_state(update, context); return
...
def run() -> None:
    ...
    try:
        ...
        logger.error("erro ao conectar/validar Google Sheets: %s", exc)
        raise SystemExit(1)

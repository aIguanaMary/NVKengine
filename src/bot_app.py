...
async def render_menu(*, update: Optional[Update], context: ContextTypes.DEFAULT_TYPE, session: Dict[str, Any], text: str, keyboard: List[List[InlineKeyboardButton]], force_new: bool = False) -> None:
    ...
    if force_new or not msg_id:
        try:
            ...
            # Se não conseguir enviar, não derruba o bot.
            pass
        return
    try:
        ...
        if "message is not modified" in str(exc).lower():
            return
        # A âncora do menu foi apagada pelo usuário (ou expirou). Recria.
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    session = get_session(update.effective_user.id)
    # /start deve SEMPRE se recuperar caso o usuário tenha apagado a conversa.
    # Então não reutilizamos o message_id anterior do menu.
    reset_flow(session, keep_menu=False)
...
async def cmd_admin(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Mesma regra do /start: não assume que a mensagem-âncora ainda existe.
    reset_flow(session, keep_menu=False)
